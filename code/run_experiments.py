...
def output_filename(output_dir, model_path, topology, k, n_rounds=None, n_agents=None, thinking=False):
    ...
    m_str = f"_N{n_agents}" if n_agents is not None else ""
...
def run(args):
    ...
    for benchmark_name in args.benchmarks:
        ...
        if args.task_ids:
            _want = set(args.task_ids.split(","))
            tasks = [t for t in tasks if str(t.get("id")) in _want]
            print(f"--task-ids filter: {len(tasks)} of the loaded tasks match "
                  f"{sorted(_want)}", flush=True)
                ...
                for rep in range(args.n_reps):
                    for task in tasks:
                        ...
                        try:
                            ...
                            # SWE centralized: the orchestrator CURATES the final
                            # patch by writing it itself in a clean lead worktree,
                            # not best-of-N and not the raw union. Active in either
                            # lead-curation mode (workers share the repo) or isolated
                            # mode (per-worker worktrees). Wire the hook exposing the
                            # team's candidate edits to the curation step.
                            # Synthesis (orchestrator curation) only under the
                            # lead-curation flag. The isolated flag alone = pure
                            # best-of-N over per-agent worktrees (the anti-N-collapse
                            # deployment: no shared-tree contention, no synth loop).
                            if (benchmark_name == "swebench"
                                    and topology_name == "centralized"
                                    and getattr(bench, "_lead_curation", False)):
                                extra_kwargs["synthesis_react"] = True

                                def _worker_artifacts_hook(_iid=task["instance_id"]):
                                    entry = bench._work_dirs.get(_iid) or {}
                                    if entry.get("lead_curation"):
                                        # Workers share one repo -> one combined diff.
                                        shared_exec, _ = entry["shared"]
                                        try:
                                            return [shared_exec.get_patch() or ""]
                                        except Exception:
                                            return [""]
                                    agents = entry.get("agents", [])
                                    nw = entry.get("n_workers", len(agents))
                                    out = []
                                    for e, _wd in agents[:nw]:
                                        try:
                                            out.append(e.get_patch() or "")
                                        except Exception:
                                            out.append("")
                                    return out

                                extra_kwargs["worker_artifacts_hook"] = _worker_artifacts_hook

                                def _seed_lead_hook(patch, _iid=task["instance_id"]):
                                    return bench.seed_lead_worktree(_iid, patch)

                                extra_kwargs["seed_lead_hook"] = _seed_lead_hook
                            ...
                            if benchmark_name == "fanoutqa":
                                ...
                                # Persist which aggregation produced the scored patch
                                # (lead_curation | shared_union | best_of_n) so runs
                                # are auditable from the JSONL, not just the traces.
                                result["swe_aggregation"] = swe_result.get("aggregation")
                            ...
                            # C1 (adaptive): also score the cheap-tier answer so
                            # the offline threshold sweep needs no re-eval. Only
                            # answer-string benchmarks (FanOutQA, BrowseComp+).
                            adaptive_block = result.get("adaptive")
                            if adaptive_block is not None:
                                ca = adaptive_block.get("cheap_answer", "")
                                if benchmark_name == "fanoutqa":
                                    from benchmarks_fanoutqa import evaluate_answer as _fa
                                    cc, cl = _fa(task, ca)
                                elif benchmark_name == "browsecomp_plus":
                                    from benchmarks_browsecomp import evaluate_browsecomp as _bc
                                    _r = _bc(task["answer"], ca)
                                    cc, cl = _r["correct"], float(_r["substring_match"])
                                else:
                                    cc, cl = None, None
                                adaptive_block["cheap_correct"] = cc
                                adaptive_block["cheap_loose"] = cl
                                adaptive_block["escalate_correct"] = is_correct
                                adaptive_block["escalate_loose"] = loose_accuracy
                            ...
                            record = {
                                ...
                                "hetero_k": result.get("hetero_k"),
                                "adaptive": adaptive_block,
                                "swe_aggregation": result.get("swe_aggregation"),
...
def main():
    ...
    parser.add_argument("--task-ids", type=str, default=None,
                        help="Comma-separated task ids: run ONLY these (after the "
                             "n-tasks load). Lets two jobs split distinct tasks to "
                             "run in parallel.")
