...
def load_benchmark(name, repo_path=None, **kwargs):
    ...
    if name == "fanoutqa":
        # MAS_CHAINED_SHARDS=1: chained-shard positive control (2026-07-16).
        # Sequentially dependent hops gated by unlock_step; default path unchanged.
        if os.environ.get("MAS_CHAINED_SHARDS", "0") not in ("", "0"):
            from benchmarks_fanoutqa import FanOutQAChainedBenchmark
            return FanOutQAChainedBenchmark()
